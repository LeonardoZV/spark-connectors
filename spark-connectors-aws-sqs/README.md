...
- **may** have a column called **message_id** (string) containing the id of each message. If not specified, this library will generate a random uuid for each message as it is required by the AWS SQS SendMessageBatch API.
...
- **may** have a column called **message_system_attributes** (map of [string, string]). Each key/value wil be added as a [metadata attribute](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-message-metadata.html) to the SQS message.
